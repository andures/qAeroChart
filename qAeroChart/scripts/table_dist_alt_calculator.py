"""CDFA Distance/Altitude Calculator table — layout insertion script (Issue #99).

Usage inside QGIS Python console:
    from qAeroChart.scripts import table_dist_alt_calculator
    table_dist_alt_calculator.run(iface)
"""
...
from qAeroChart.ui.dist_alt_calculator_dialog import DistAltCalculatorDialog

TABLE_NAME = "dist_alt_calculator_table"
TABLE_ID_PREFIX = "dist_alt_calc_table_"


# ------------------------------------------------------------------
# Internal helpers (mirror table_gs_rod.py)
# ------------------------------------------------------------------
...
def _classify_rows(table_rows: list[list[str]]) -> list[str]:
    """Classify each row as 'title' or 'data' (first row with a non-numeric label + filled
    remaining columns is the header; everything else is a data row)."""
    header_idx = None
    for i, row in enumerate(table_rows):
        if len(row) > 2 and any(row[j] for j in range(1, len(row))):
            header_idx = i
            break
    types = []
    for i, row in enumerate(table_rows):
...
def build_dialog(iface=None, parent=None, default_layout_name: str | None = None) -> DistAltCalculatorDialog:
    """Create and configure the dialog without showing it (non-blocking flow)."""
    dlg = DistAltCalculatorDialog(iface=iface, parent=parent)
    if default_layout_name:
        dlg.select_layout(default_layout_name)
    return dlg


def insert_from_dialog(dlg: DistAltCalculatorDialog, iface=None) -> None:
    ...
    layout.refresh()
    n_rows = max(len(table_rows) - 1, 0)  # subtract header row
    if iface:
        iface.messageBar().pushInfo(
            "Distance/Altitude Calculator",
            f"Table added to layout '{layout.name()}' with {n_rows} distance rows.",
        )
    else:
        print(f"Table added to layout '{layout.name()}' with {n_rows} distance rows.")
